...
function findEmployeeById(employeeId) {
 ...
 {
    const foundEmployee = employees.find(employee => employee.speciaization === employeeSpeciaization);
    if (foundEmployee) {
    document.getElementById('employeesDetails').innerHTML =`<p>${foundEmployee.id}: ${foundEmployee.name} - ${foundEmployee.department} - ${foundEmployee.speciaization}</p>`;
    }
    else{
